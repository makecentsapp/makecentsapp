...
try {
    ...
    if ($already_registered !== true) {
        ...
        $handle=fopen('license.php', "w+");
        fclose($handle);
    }
    $license  = file_get_contents('license.php');
} catch (Exception $e) {
    var_dump($e);
...
if (isset($_POST['registration-form']) && $already_registered === false) {
    ...
    if (isset($result->error_msg)) {
        ...
        if ($result->notification_case !== "notification_license_ok") {
            ...
            try {
                file_put_contents('license.php', $result->notification_text);
                ...
                var_dump($e);
                exit('Unable to write file license.php - please check your write access (chmod)');
            }
        }
    } else {
