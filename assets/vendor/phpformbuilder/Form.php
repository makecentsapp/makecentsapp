...
class Form
{
    ...
    */
    ...
    public function addJqueryFileUpload($type, $name, $value = '', $label = '', $attr = '', $fileUpload_config = '')
    {
        ...
        if (!empty($label)) {
            $start_label .= '<label for="' . $uploaderId . '"' . $this->getLabelClass('fileinput') . '>';
            if (in_array(str_replace('[]', '', $name), array_keys($this->error_fields))) {
                $start_label .= '<span class="' . $this->options['textErrorClass'] . '">' . $this->getRequired($label, $attr) . '</span>';
            } else {
                $start_label .=$this->getRequired($label, $attr);
    ...
    public function addPlugin($plugin_name, $selector, $js_config = 'default', $js_replacements = array())
    {
        ...
        if ($plugin_name == 'material-datepicker' || $plugin_name == 'material-timepicker') {
            ...
            if (!in_array('material-pickers-base', $this->js_plugins)) {
                $this->addPlugin('material-pickers-base', '#', 'default', array('%pluginsUrl%' => $this->plugins_url));
            }
    ...
    */
    ...
    public static function checkRegistration($context = 'object')
    {
        if (!file_exists(__DIR__ . '/license.php')) {
            try {
                if (!@file_put_contents(__DIR__ . '/license.php', '')) {
                    throw new \Exception('Unable to write License file to ' . str_replace(array('\\', '/'), DIRECTORY_SEPARATOR, __DIR__ . '/license.php') . '.<br><a href="http://php.net/manual/en/function.chmod.php" target="_blank" style="color:#fff;text-decoration:underline;">Increase your chmod</a> to give write permission to this folder.', 1);
                }
            } catch (\Exception $e) {
        ...
        $license  = file_get_contents(__DIR__ . '/license.php');

        if (preg_match('`^<ROOT_URL>(http|https)://(?:www.)?' . $_SERVER['HTTP_HOST'] . '[^<]+</ROOT_URL>`', $license)) {
            return true;
        }
    ...
    public static function testToken($form_ID)
    {
        ...
        if (TOKEN_CONFIG != '885kufR**.xp5e6S' || sha1_file(__DIR__ . '/Validator/Token.php') != '99d96ef538f0914e6bbfb97fff27a7b23e26282a') {
            exit('error');
    ...
    */
    ...
    protected function getJsCode()
    {
        ...
        $this->js_code  = '<script type="text/javascript"' . $defer . '>' . "\n";
        $this->js_code .= 'if (typeof forms === "undefined") {';
        $this->js_code .= '    var forms = [];';
        $this->js_code .= '}';
        $this->js_code .= $this->options['openDomReady'] . "\n";
        $this->js_code .= '    if (top != self) {$(\'form\').on(\'submit\', function(e) {e.preventDefault();return false;});}';
    ...
    public function setPluginsUrl($forced_url = '')
    {
        ...
        if (!empty($forced_url)) {
            ...
            $form_class_path    = strtolower(dirname(__FILE__));
